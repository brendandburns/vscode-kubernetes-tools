...
export async function init(registry: clusterproviderregistry.ClusterProviderRegistry, context: google.Context): Promise<void> {
    if (!registered) {
        registry.register({id: 'gke', displayName: "Google Kubernetes Engine", supportedActions: ['create', 'configure'], next: (w, a, m) => next(context, w, a, m)});
        registered = true;
    }
}
...
function next(context: google.Context, wizard: Wizard, action: clusterproviderregistry.ClusterProviderAction, message: any): void {
    wizard.showPage("<h1>Contacting Google</h1>");
...
function getHandleCreateHtml(step: string | undefined, context: google.Context, requestData: any): Sequence<string> {
    if (!step) {
        return promptForProject(requestData, context, "create", "metadata");
        ...
        return renderInternalError(`googleStepError (${step})`);
    }
}

function getHandleConfigureHtml(step: string | undefined, context: google.Context, requestData: any): Sequence<string> {
    if (!step) {
        return promptForProject(requestData, context, "configure", "cluster");
...
async function promptForProject(previousData: any, context: google.Context, action: clusterproviderregistry.ClusterProviderAction, nextStep: string): Promise<string> {
    const projectList = await google.getProjectList(context);
    if (!projectList.result.succeeded) {
        return renderCliError('PromptForProject', projectList);
    }

    const projects: string[] = projectList.result.result;

    if (!projects || !projects.length) {
        return renderNoOptions('No google projects', 'You have no google projects.');
    }

    const options = projects.map((s) => `<option value="${s}">${s}</option>`).join('\n');
    return formPage({
        stepId: 'PromptForProjects',
        title: 'Choose project',
        waitText: 'Contacting Google',
        action: action,
        nextStep: nextStep,
        submitText: 'Next',
        ...
        formContent: `
            <p>
            google project: <select name='project' id='selector'>
            ${options}
            </select>
            </p>

            <p><b>Important! The selected project will be set as the active project for the google CLI.</b></p>
        `
    });
}

async function promptForCluster(previousData: any, context: google.Context): Promise<string> {
    const clusterList = await google.getClusterList(context, previousData.project);
    ...
    if (!clusters || clusters.length === 0) {
        return renderNoOptions('No clusters', 'There are no Kubernetes clusters in the selected project.');
    }
...
async function configureKubernetes(previousData: any, context: google.Context): Promise<string> {
    const selectedCluster = parseCluster(previousData.cluster);
    const configureResult = await google.configureCluster(context, selectedCluster.name, selectedCluster.resourceGroup);
    await refreshExplorer();
    return renderConfigurationResult(configureResult);
}

async function promptForMetadata(previousData: any, context: google.Context): Promise<string> {
    const serviceLocations = await google.listGkeLocations(context);
    ...
    return formPage({
        ...
        title: 'google cluster settings',
        waitText: 'Contacting Microsoft google',
        action: 'create',
...
async function promptForAgentSettings(previousData: any, context: google.Context): Promise<string> {
    const vmSizes = await google.listVMSizes(context, previousData.location);
...
async function createCluster(previousData: any, context: google.Context): Promise<string> {
    ...
    const createResult = await google.createCluster(context, options);
    ...
    const message = succeeded(createResult.result) ?
        `<div id='content'>
         ...
         <p class='success'>google is creating the cluster, but this may take some time. You can now close this window,
...
function waitForClusterAndReportConfigResult(previousData: any, context: google.Context): Observable<string> {

    async function waitOnce(refreshCount: number): Promise<[string, boolean]> {
        const waitResult = await google.waitForCluster(context, previousData.clustername);
        ...
        const configureResult = await google.configureCluster(context, previousData.clustername, previousData.resourcegroupname);
...
function renderConfigurationResult(configureResult: ActionResult<google.ConfigureResult>): string {
    const title = configureResult.result.succeeded ? 'Cluster added' : `Error ${configureResult.actionDescription}`;

    const result = configureResult.result as Succeeded<google.ConfigureResult>;  // currently always reports success and puts failure in the body
    const configResult = result.result;
    const clusterServiceString = result.result.clusterType === "aks" ? "google Kubernetes Service" : "google Container Service";
...
function diagnoseCreationError(e: Errorable<Diagnostic>): string {
    ...
    if (e.error[0].indexOf('unrecognized arguments') >= 0) {
        return '<p>You may be using an older version of the google CLI. Check google CLI version is 2.0.23 or above.<p>';
    }
    return '';
}
...
function diagnoseCreationSuccess(e: Errorable<Diagnostic>): string {
    ...
    return `<p><b>Note:<b> although google accepted the creation request, the google CLI reported the following message. This may indicate a problem, or may be ignorable progress messages:<p>
        <p>${error}</p>`;
}
...
function renderCliError<T>(stageId: string, last: ActionResult<T>): string {
    ...
    return `<!-- ${stageId} -->
        ...
        <p><span class='error'>The google command line failed.</span>  See below for the error message.  You may need to:</p>
        <ul>
        <li>Log into the Google CLI (run gcloud login in the terminal)</li>
        <li>Install the Google CLI <a href='https://cloud.google.com/sdk/install'>(see the instructions for your operating system)</a></li>
        </ul>
        <p><b>Details</b></p>
        <p>${errorInfo.error}</p>`;
}
...
<p>This is not an google or Kubernetes issue.  Please report error text '${error}' to the extension authors.</p>
`;
}
...
function parseCluster(encoded: string): google.ClusterInfo {
